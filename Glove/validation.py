...
def prepare_data(df, vocab, embeddings, mode='mean'):
    ...
    for idx, line in df.iterrows():
        if mode == 'mean':
            ...
            X.append(text_to_embedding_padding(line['sentence1'], vocab, embeddings, 50))
        y.append(line['label'])
        ...
        for epoch in range(30):
            ...
            if epoch % 5 == 0:
                if x_val is not None and y_val is not None:
                    y_val_pred = self.predict(x_val, device)
...
if __name__ == "__main__":
    ...
    rnn_classifier.fit(X_train_rnn, y_train, device, X_val_rnn, y_val)
    y_pred = rnn_classifier.predict(X_test_rnn, device)
